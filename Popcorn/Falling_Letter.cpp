...
void AFalling_Letter::Draw(HDC hdc, RECT &paint_area)
{
   ...
   if (IntersectRect(&intersection_rect, &paint_area, &Letter_Cell) ) // Рисуем букву с его подложкой только если текущая область рисования пересекается с прямоугольником этой буквы
...
{
   rect = Letter_Cell;  // Теперь мы можем не открывать доступ к прямоугольнику а все значения прямоугольника автоматически перейдут в значения для прямоугольника rect так как мы присвоили ему значения этого прямоуг
}
   ...
   else
   {
      ...
      if (Rotation_Step > 4 && Rotation_Step <= 12)
      {
         ...
         case ELT_I: // "Інверсія"
            ...
            //LineTo (hdc, 7 * AsConfig::Global_Scale + 1, 1 * AsConfig::Global_Scale - Brick_Half_Height - 1);
            //MoveToEx(hdc, 7 * AsConfig::Global_Scale + 1, 3 * AsConfig::Global_Scale - Brick_Half_Height, 0);
            //LineTo (hdc, 7 * AsConfig::Global_Scale + 1, 6 * AsConfig::Global_Scale - Brick_Half_Height - 1);
            Draw_Line(hdc, 7, 1, 7, 1);
            Draw_Line(hdc, 7, 3, 7, 6);
            break;
